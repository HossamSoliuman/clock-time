...
<section class="convertSection py-5">
    <div class="container">
        <div class="text-center">
            <h2>Effortless Conversion: City, GMT/UTC, and Time Zone Transformations</h2>
        ...
        <div class="w-75 m-auto mt-5">
            <form id="timeZoneForm2" action="{{ url('convert-time') }}" method="GET"
                onsubmit="return changeAction3(event);">
                <div class="row">
                    <div class="col-lg-5 mb-3 position-relative">
                        ...
                        <select class="select-zone select4 w-100" name="search_1">
                            <option value="" disabled selected>Select City,GMT/UTC,Time Zone </option>
                        </select>
                    </div>
                    <div class="col-lg-5 mb-3 position-relative">
                        <i class="fa-solid fa-location-dot position-absolute"
                            style="z-index: 10;left:20px;top:12px"></i>
                        <select class="select-zone select4 w-100" name="search_2">
                            <option value="" disabled selected>Select City,GMT/UTC,Time Zone </option>
                        </select>
                    </div>
                    <div class="col-lg-2">
...
<script>
    $(document).ready(function() {
        $('.select4').select2({
            placeholder: "Select City,GMT/UTC,Time Zone",
            allowClear: true,
            ajax: {
                url: "{{ route('fetch.all') }}",
                dataType: 'json',
        ...
        }).on('select4:open', function() {
            // Trigger the select2 AJAX request manually when opened without input
            const $this = $(this);
            const searchTerm = $this.data('select4').dropdown.$search.val();
...
<script>
    function changeAction3(event) {
        event.preventDefault(); // Prevent the default form submission

        // Get the selected values from the dropdowns
        const gmtSelect = document.querySelector('select[name="search_1"]');
        const timezoneSelect = document.querySelector('select[name="search_2"]');
        ...
        if (gmtValue && timezoneValue) {
            ...
            const newUrl = `/convert-all/${gmtValue}/to/${timezoneValue}`;

            // Redirect to the new URL
            window.location.href = newUrl;
        } else {
            alert('Please select both City,GMT/UTC,Time Zone.'); // Alert if not selected
        }
